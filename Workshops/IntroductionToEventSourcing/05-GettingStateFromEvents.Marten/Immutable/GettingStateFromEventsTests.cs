...
{
    private ShoppingCart() : this(Guid.Empty, Guid.Empty, ShoppingCartStatus.Pending, Array.Empty<PricedProductItem>()) { } // Let's make Marten happy
...
public class GettingStateFromEventsTests: MartenTest
{
        ...
        CancellationToken cancellationToken)
    {
        // 1. Add logic here
        //My Approach
        //var events = await documentSession.Events.FetchStreamAsync(shoppingCartId, token: cancellationToken);
        //return ShoppingCart.From(events.Select(x => (ShoppingCartEvent)x.Data).ToArray());

        var shoppingCart = documentSession.Events.AggregateStreamAsync<ShoppingCart>(shoppingCartId, token: cancellationToken);
